...
class WorkerNode:
    ...
    def apply_command(self, command: dict[str, Any]) -> None:
        ...
        if msg_type == TYPE_COMMAND_REDIRECT:
            ...
            new_master_id = payload.get("new_master_id") or new_master_address
    ...
    def worker_master_loop(self, stream) -> None:
        while not self.stop_event.is_set():
            ...
            command_applied = self.request_task_or_command(stream)
            if command_applied:
                return
            time.sleep(self.interval)
    ...
    def request_task_or_command(self, stream) -> bool:
        payload = {
            "WORKER": WORKER_ALIVE,
            "WORKER_UUID": self.worker_id,
    ...
    def register_temporary_worker(self, stream) -> None:
        ...
        if isinstance(ack.get("master_id"), str) and ack["master_id"]:
            self.current_master_id = ack["master_id"]
            log(self.worker_id, f"ID do Master atual atualizado via ACK: {self.current_master_id}")
