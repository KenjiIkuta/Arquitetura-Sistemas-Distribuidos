...
class MasterNode:
    ...
    def start(self) -> None:
        ...
        supervisor_thread = threading.Thread(target=self.metrics_supervisor_loop, daemon=True)
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
            ...
            log(self.master_id, f"capacity={self.capacity} release_threshold={self.release_threshold}")
            log(self.master_id, f"tasks_iniciais={self.task_queue.qsize()}")
            log(self.master_id, f"vizinhos={list(self.neighbors.keys()) or 'nenhum'}")
            log(self.master_id, "Pressione Ctrl+C para encerrar.")
    ...
    def handle_heartbeat(self, payload: dict[str, Any], stream, addr: tuple[str, int]) -> None:
        received_server_uuid = payload.get("SERVER_UUID")
        if received_server_uuid and received_server_uuid != self.master_id:
            log(self.master_id, f"Heartbeat de {addr} com SERVER_UUID={received_server_uuid} (esperado={self.master_id}); respondendo ALIVE para interoperabilidade")
        ...
        log(self.master_id, f"Heartbeat respondido ALIVE para {addr}")
    ...
    def handle_typed_message(self, message: dict[str, Any], stream, addr: tuple[str, int]) -> None:
        ...
        if msg_type == TYPE_REGISTER_TEMPORARY_WORKER:
            ...
            send_json_line(stream, {"STATUS": STATUS_ACK, "WORKER_UUID": payload.get("worker_id"), "request_id": request_id, "master_id": self.master_id})
            return
    ...
    def metrics_supervisor_loop(self) -> None:
        while not self.stop_event.is_set():
            ...
            self.stop_event.wait(METRICS_INTERVAL)

    def _build_metrics_payload(self) -> dict[str, Any]:
        ...
        try:
            ...
            load_1m, load_5m = round(load_avg[0], 2), round(load_avg[1], 2)
        except (AttributeError, OSError):
            load_1m, load_5m = 0.0, 0.0

        with self.lock:
            all_workers = dict(self.workers)
            local_workers_snap = set(self.local_workers)
            borrowed_workers_snap = dict(self.borrowed_workers)
            outgoing_loaned_snap = dict(self.outgoing_loaned_workers)
            queue_size = self.task_queue.qsize()
            tasks_completed = self.total_completed
            tasks_failed = self.total_failed
            neighbors_snap = list(self.neighbors.values())

        busy_count = sum(1 for s in all_workers.values() if s.busy)
        total_registered = len(all_workers)
        workers_idle = max(0, total_registered - busy_count)

        # Lista de workers emprestados: "out" = cedemos a outro, "in" = recebemos de outro
        borrowed_list: list[dict[str, str]] = []
        seen_out: set[str] = set()
        for borrower_id in outgoing_loaned_snap.values():
            if borrower_id not in seen_out:
                borrowed_list.append({"direction": "out", "peer_uuid": str(borrower_id)})
                seen_out.add(borrower_id)
        seen_in: set[str] = set()
        for state in borrowed_workers_snap.values():
            peer = state.borrowed_from_master_id or "UNKNOWN"
            if peer not in seen_in:
                borrowed_list.append({"direction": "in", "peer_uuid": peer})
                seen_in.add(peer)
        ...
        return {
            "server_uuid": self.master_id,
            "hostname": f"{self.master_id}.farm.local",
            "role": "master",
            ...
            "performance": {
                ...
                "farm_state": {
                    "workers": {
                        ...
                        "workers_utilization": busy_count,
                        "workers_alive": total_registered,
                        "workers_idle": workers_idle,
                        "workers_borrowed": len(outgoing_loaned_snap),
                        "workers_received": len(borrowed_workers_snap),
                        "workers_failed": 0,
                        "workers_home": len(local_workers_snap) + len(outgoing_loaned_snap),
                        "workers_available_capacity": workers_idle,
                        "borrowed_workers": borrowed_list,
                    },
                    "tasks": {
                        "tasks_pending": queue_size,
                        "tasks_running": busy_count,
